...
int main()
{
	...
	Process * p1 = system.createProcess();
	p1->createSegment(0, 1, READ_WRITE);


	p1->createSharedSegment(1024, 1, "prvi", READ_WRITE);

	Status status = system.access(p1->getProcessId(), 12, WRITE);
	if (status == PAGE_FAULT)
	{
		p1->pageFault(12);
	}
	PhysicalAddress paddr = p1->getPhysicalAddress(12);
	std::cout << "Adresa je " << paddr << "\n";
	*(char *)paddr = (char)-1;


	Process * p2 = system.cloneProcess(p1->getProcessId());
	status = system.access(p2->getProcessId(), 12, WRITE);
	if (status == PAGE_FAULT)
	{
		p2->pageFault(12);
	}
	paddr = p2->getPhysicalAddress(12);
	std::cout << "Adresa je " << paddr << "\n";
	*(char *)paddr = (char)-1;

	Process * p3 = system.cloneProcess(p2->getProcessId());
	status = system.access(p3->getProcessId(), 12, READ);
	if (status == PAGE_FAULT)
	{
		p3->pageFault(12);
	}
	paddr = p3->getPhysicalAddress(12);
	std::cout << "Adresa je " << paddr << "\n";
	*(char *)paddr = (char)-1;

	status = system.access(p2->getProcessId(), 1030, WRITE);
	if (status == PAGE_FAULT)
	{
		p1->pageFault(1030);
	}
...
//
//	Process * p2 = system.cloneProcess(p1->getProcessId());
//	status = system.access(p2->getProcessId(), 12, READ);
//	if (status == PAGE_FAULT)
//	{
//		p2->pageFault(12);
//	}
...
//
//	Process * p3 = system.cloneProcess(p2->getProcessId());
//	status = system.access(p3->getProcessId(), 12, WRITE);
//	if (status == PAGE_FAULT)
//	{
//		p3->pageFault(12);
//	}
